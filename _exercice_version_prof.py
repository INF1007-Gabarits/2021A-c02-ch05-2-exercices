...
def format_horizontal_histogram(histogram):
	...
	# Solution sur une ligne avec compréhension de liste imbriquée. Pas vraiment plus clair, non? Moins de ligne ne veut pas nécessairement dire plus élégant.
	#return "\n".join(["".join([(BLOCK_CHAR if col >= i + 1 else " ") for col in histogram[1:]]) for i in range(height - 1, -1, -1)]) + "\n" + LINE_CHAR * (len(histogram) - 1)
